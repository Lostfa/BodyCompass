...
# ---- 跨源隔离中间件 ----
# cornerstone.js 体积渲染依赖 SharedArrayBuffer，
# 浏览器要求页面处于跨源隔离状态（COOP/COEP）。
@app.middleware("http")
...
async def startup():
    """应用启动时的初始化操作"""
    print(f"[BodyCompass] 项目根目录: {settings.project_root}")
    print(f"[BodyCompass] 管线代码目录: {settings.pipline_dir}")
    print(f"[BodyCompass] 默认工作目录: {settings.base_working_dir}")
    task_manager.start_cleanup()
    print("[BodyCompass] 任务管理器已启动")


@app.on_event("shutdown")
async def shutdown():
    """应用关闭时的清理操作"""
    print("[BodyCompass] 应用已关闭")
...
async def favicon():
    ...
    svg_icon = (
        ...
        '<text x="16" y="23" text-anchor="middle" font-size="22" fill="white">BC</text>'
        '</svg>'
    )
...
async def serve_index():
    ...
    return {"message": "BodyCompass API Server is running. Frontend not yet built."}
